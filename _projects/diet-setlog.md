...
분석 모델이 Gemini라 구글 생태계로 통일했습니다. 근거는 그 한 줄이 전부입니다.

### 두 달 뒤에 확인한 것

9월 16일에 레포를 다시 열어 문서대로 동작하는지 검증했습니다. 문서는 "익명 세션으로 전 기능 사용"이라고 하는데 앱은 카카오 로그인이 없으면 홈에 못 들어가고, 게스트 세션 API는 서버와 앱 클라이언트 양쪽에 완성된 채 어디서도 호출되지 않으며, 스펙에 적힌 레이트리밋은 구현되지 않았습니다. 자세한 건 [먼저 볼 글]({{ "/2026/09/16/anonymous-first-but-kakao-required/" | relative_url }})에 적었습니다.
