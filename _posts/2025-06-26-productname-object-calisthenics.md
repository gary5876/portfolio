...
description: "첫 코드리뷰에서 '왜 이렇게 짰나요'를 받고 답하지 못해 지운 클래스, 그리고 지우는 김에 잘못 같이 지운 예외 처리"
---
...
답이 없는 코드는 다음 날 아침 지웠습니다. 그런데 지우는 김에 지적받지 않은 예외 처리까지 같이 걷어냈고, 그 구조는 세 번째 미션에서 다시 만들게 돼요. 이 글에서는 왜 그 클래스를 만들었는지, 왜 지웠는지, 지우면서 무엇을 잘못 같이 지웠는지를 코멘트와 커밋 그대로 두고 살펴봅니다. 요구사항은 다 통과한 상태였으니 성능이나 규모 숫자는 이 글에 없습니다.
...
이 미션에서 남은 것도 적어 둡니다. 예외를 한 곳에서 받는 구조를 이유 없이 걷어낸 건 그때는 몰랐고, 세 번째 미션에서 같은 구조를 다시 만들면서 알았습니다. 저장소 분리는 "필요해지면"으로 미뤘다가 이틀 만에 필요해졌고요. 그래서 지금은 코드를 지울 때 지적받은 것에 딸려 나가는 게 없는지 먼저 보고, "나중에"라고 미룬 필요가 언제 올지도 같이 적어 둡니다.
