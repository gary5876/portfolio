...
커밋된 첫 버전이 위 코드였고, 수정 커밋 메시지에 남긴 대로 그 뒤에 오염된 전체를 읽은 뒤 test와 겹치는 행을 걸러내는 사후 필터링으로 막아 보려 했습니다. 그런데 CSV로 저장하면서 부동소수점 정밀도가 달라지거나 오염 종류에 따라 행 수가 바뀌어서 같은 행인지 판정이 흔들렸고, uniqueness와 class_balance에서 누수가 계속 남았습니다.
