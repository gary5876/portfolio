...
그날 하루 동안 그 보고 여섯 건 중 세 건이 틀렸다는 걸 알게 됐고, 세 건 다 제가 따져 묻고 나서야 드러났어요. 여섯 건이 무엇이었고 그중 뭐가 틀렸는지는 레포의 `04-purpose-and-self-improvement-loop.md`에 정리돼 있습니다.

| 맡긴 조사 | 결과 |
|---|---|
| OSS 레퍼런스 1차 서베이(4개 요소) | "좁고 명시적인 인터페이스" 항목이 "라이브러리 없음"으로 틀림 |
| data lake 레퍼런스 재조사 | Nessie 1순위 추천이 틀림 |
| filter 레퍼런스 재조사 | 맞음, 재검증에서 수치 일치 |
| 방향 조사(수익형 대 서비스형) | 맞음, 다만 "레딧은 없냐"는 지적으로 추가 조사 요청 |
| 커뮤니티 조사(레딧, HN) | 맞음, Reddit 자체는 도구 한계로 못 가져왔고 그 한계를 숨기지 않고 보고함 |
| 엣지케이스, 트레이싱, 재시도 조사 | LangGraph를 이미 채택했다는 전제가 틀림 |

*표 1. 그날 맡긴 조사 여섯 건과 결과. 부정 결론("없다")과 순위 결론("1순위")이 셋 다 틀렸습니다.*
...
이 항목만 왜 틀렸는지는 나중에 적어 둔 기록이 있습니다. "1차 서베이는 일반화된 narrow-interface 프레임워크는 없고, github-mcp-server 하나만 참고할 패턴이라고 결론 냈다"인데, 제가 "레퍼런스 조사의 성실성을 재검증하라"고 요구해 `gh api`로 직접 다시 뒤지니 결론이 틀렸습니다. modelcontextprotocol/servers(90,019★), github-mcp-server(32,665★), cloudflare/mcp-server-cloudflare(4,137★), grafana/mcp-grafana(3,407★), crystaldba/postgres-mcp(3,246★), mongodb-js/mongodb-mcp-server(1,118★), 여섯 개가 나왔어요. 1차 조사가 "없다"고 결론 낼 만큼 깊이 찾지 않은 채로 끝낸 것이었습니다.
...
커뮤니티 조사가 추가로 나왔고, 거기서 보고된 문제들을 다 모아 보라고 했더니 문제 하나만 골라서 써 오길래 다시 다 모으게 했습니다. 그렇게 모은 문제 목록으로 후속 조사를 돌렸는데, 그 조사가 "이미 채택한 LangGraph"를 전제로 쓰여 있었어요. LangGraph를 채택한 적이 없었습니다. 그날 오후 방향 조사에서 "활발하고 실재하는 프레임워크"라고 좋게 언급된 게 전부였는데, 다음 조사가 그걸 결정으로 받아서 그 위에 쌓은 거였죠. 문서에는 "오해였다"로 바로잡아 적었습니다. 같은 문서에 data lake 조사도 나옵니다. 1차 재조사가 Nessie(1.5천 스타)를 1순위로 추천했는데, 제가 "높은 수준, 지속된 유지보수, 유명함" 기준으로 재검증을 요구하자 2차 조사에서 Nessie가 2026년 10월까지 Apache Polaris로 합병 예정이라는 사실이 나왔습니다. 단독 프로젝트인 것처럼 1순위로 추천한 게 유지보수 지속성을 안 본 오판이었어요.
...
솔직히 말하면 이 조사도, 정리도, 규칙을 문서로 옮긴 것도 Claude가 했습니다. 제가 한 건 미덥지 않을 때 따져 묻고, 보고와 원본을 대조한 것뿐이에요. 그리고 그 확인 전담 역할은 만들어 둔 뒤로 그 세션에서 한 번도 그 이름으로 불리지 않았습니다. 오케스트레이터 패턴을 정리한 문서(`06-orchestrator-pattern.md`)에 그 상태가 그대로 적혀 있어요. 검증 단계는 "verifier 정의됨, 아직 코디네이터가 직접 수행 중"이라고, 반영 단계도 "doc-writer 정의됨, 아직 코디네이터가 직접 수행 중"이라고 표시돼 있습니다. `.claude/agents/`에 파일로는 있는데 그 이름으로 실제로 불린 적이 없다는 뜻이에요. 왜 안 불렸는지는 그 문서에도 이유가 없습니다. 다만 같은 문서가 스스로 인정한 한계는 있어요. "진짜 병렬 실행을 검증한 적이 없다, 여섯 번 다 순차적으로 fork 하나씩 띄우고 끝날 때까지 기다린 뒤 다음 걸 던졌다." 역할을 나눠 만들어 놓고도 그 세션 안에서는 병렬로도, 이름을 불러서도 쓰지 않았다는 겁니다. 세 건을 잡은 건 규칙이 아니라 제 질문이었고, 그 구조는 아직 그대로입니다.
