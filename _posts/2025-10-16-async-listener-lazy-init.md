...
description: "제출은 됐는데 이메일이 조용히 안 나가던 장애. 원인은 이틀 전 내가 되돌린 트랜잭션, 해법은 엔티티 대신 값 넘기기"
---

안녕하세요, 고준서입니다. 동아리움이라는 대학 동아리 지원 서비스의 백엔드를 팀에서 맡고 있고(백엔드 3인, 프론트엔드 3인), 지원서를 내면 나가는 알림 이메일은 제가 담당했습니다. 2025년 10월 16일 저녁에 그 메일이 한 통도 안 나가고 있다는 걸 운영 로그에서 알았어요. 지원서는 멀쩡히 저장되고 화면에는 "처리됨"까지 뜨는데 메일만 안 나가는 상태였습니다. 원인은 메일을 보내는 비동기 스레드가 DB 세션이 닫힌 뒤에 엔티티를 건드린 것이었는데, 그 조건을 만든 건 이틀 전의 저였습니다. 한 번 고쳤다가 42분 만에 되돌린 PR이 있었거든요.

그래서 이 글은 로그 한 줄에서 원인까지 간 순서와, 그 이틀 전 PR 두 개(#136, #137)에서 무슨 일이 있었는지, 그리고 왜 리스너에 트랜잭션을 다시 붙이는 대신 엔티티 대신 값을 넘기기로 했는지를 이슈와 PR 번호를 따라가며 적었습니다. 그 이틀 동안 메일이 안 나간 지원서가 몇 건인지는 지금도 모릅니다. 셀 장치가 없었고, 그 이야기도 뒤에 나옵니다.
...
나중에 [이슈 #144](https://github.com/kakao-tech-campus-3rd-step3/Team18_BE/issues/144)에 재현 방법을 이렇게 적었습니다.
...
로그에 찍힌 스레드 이름이 `task-2`입니다. 사용자의 요청을 처리하는 스레드가 아니라, 메일 발송용으로 따로 도는 비동기 스레드예요. 이 프로젝트는 지원서가 DB에 저장(커밋)된 뒤에 이벤트를 하나 발행하고, 그 이벤트를 받은 리스너가 다른 스레드에서 메일을 보내는 구조였습니다. 제출 응답이 메일 서버를 기다리지 않게 하려고 그렇게 떼어 놓은 거였고, 그 결정은 [앞 글]({{ "/blog/2025/10/08/email-event-retry-classification/" | relative_url }})에 있습니다.
...
남은 것도 있습니다. 비동기 스레드의 실패를 세는 지표는 지금도 없고, 기본 핸들러가 남기는 ERROR 로그 한 줄이 전부예요. 그래서 앞에서 말한 대로, 그 이틀 동안 안 나간 메일이 몇 통인지 셀 방법이 없습니다. 재시도 분류기는 메일 서버의 응답 코드만 보기 때문에, 발송기 앞에서 나는 예외는 여전히 분류 밖입니다. 리스너에는 이제 쓰지 않는 `ApplicationRepository` 주입이 남아 있고, 이메일 통합 테스트는 실제 메일 서버로 보내는 방식이라 꺼져 있어서, 비동기 스레드에서 연관 필드가 터지는 이 경로를 자동으로 재현하는 테스트는 없습니다. 같은 조건이 다시 오면 그때처럼 로그에서 알게 될 거예요.

이 사고에서 제가 다시 꺼내 쓰는 건 이렇습니다.

1. `@Async` 리스너 안에서는 엔티티의 연관 필드를 읽을 수 없습니다. 세션이 이미 닫혀 있으니, 값은 이벤트를 발행하는 쪽에서 꺼내 넘겨야 합니다.
2. 비동기 스레드의 예외는 요청을 보낸 쪽으로 돌아오지 않습니다. 핸들러나 지표가 없으면 사용자는 성공 화면을 보고 팀은 모릅니다.
3. 고친 것을 되돌릴 때는 이유를 한 줄이라도 남겨야 합니다. #137에 그 한 줄이 없어서 "왜 되돌렸나"는 지금도 추측입니다.

그 로그를 여드레 뒤가 아니라 그날 팀에 올리는 것이 이 사고에서 제가 가져가는 숙제입니다. 읽어주셔서 감사합니다.
