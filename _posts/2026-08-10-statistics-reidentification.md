...
description: "항목별로 가리면 뺄셈으로 복원된다. 지원자 3명 미만이면 분포를 통째로 감추기로 한 이유와 그 구현이 틀렸던 자리"
---

안녕하세요, 고준서입니다. 동아리움이라는 대학 동아리 지원 서비스의 백엔드를 팀(백엔드 3인, 프론트엔드 3인)에서 맡고 있고, 지원자 통계 기능은 제가 전담했어요. 지원폼별 성별, 학부, 학번, 일자별 지원 추이를 지원자에게 공개하는 API인데, 공개 통계는 누가 지원했는지 알아낼 수 있는 자료가 되기도 합니다. 8월 5일부터 10일 사이에 그 부분을 잡았는데, 결론은 항목을 하나씩 가리는 게 아니라 지원자가 3명이 안 되면 분포를 통째로 감추는 것이었어요. 이 글에는 그 기준이 어떻게 정해졌는지, 항목별로 가리던 구현이 왜 틀렸는지를 PR과 제 대화 기록으로 확인할 수 있게 적었습니다. 지원폼당 지원자가 보통 몇 명인지 같은 규모 숫자는 없습니다. 당시 재지 않았거든요. 코드 초안은 Claude Code가 썼고 저는 읽고 되묻는 쪽이었습니다.
...
항목별로 가리는 구현은 가린 값을 null로 바꿨는데, 그러면 화면에서 "지원자가 적어서 가린 것", "실제로 그날 0명", "학부를 입력 안 한 것"이 전부 빈 값으로 보입니다. 셋을 구분하게 했어요. 가린 경우는 응답 최상위에 `masked: true`와 빈 결과, 실제 0명은 항목의 `count: 0`, 미입력은 `UNKNOWN` 항목. 기준값 3은 설정으로 뺐습니다. 왜 3인지는 이슈에도 PR에도 없습니다. [이슈 #335](https://github.com/kakao-tech-campus-3rd-step3/Team18_BE/issues/335) 원안은 항목별 임계값 5였고, 총합 기준으로 바꾸면서 3이 됐는데 그 숫자를 고른 이유는 적어 두지 않았어요.
...
지금 남은 약점은 기준값 3이 작다는 겁니다. 지원자가 4명이면 분포가 그대로 공개되는데, 4명 중 1명뿐인 학부 항목은 사실상 그 사람이에요. 그리고 통계를 반복해서 조회하면 지원 전후의 차이로 한 사람의 속성 조합을 추정할 수 있는데, 이건 총합 기준으로도 막히지 않습니다. 지금은 30분짜리 캐시가 갱신 단위를 뭉개 주는 걸로 완화될 뿐이에요. 그 캐시를 왜 스케줄러 대신 요청 시점 계산으로 갔는지는 [다음 글]({{ "/blog/2026/08/18/statistics-cache-aside/" | relative_url }})에 적었습니다.
