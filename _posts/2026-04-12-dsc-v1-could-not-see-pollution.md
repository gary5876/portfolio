...
안녕하세요, 고준서입니다. 4학년 캡스톤은 팀으로 했고, 데이터 품질 점수 엔진(DSC)과 실험 파이프라인은 제가 맡았습니다. 캡스톤의 질문은 "데이터에 매긴 품질 점수가 그 데이터로 학습한 모델의 성능을 예측하는가"였는데, 그 질문에 답하려면 먼저 점수가 오염에 반응은 하는지부터 확인해야 했습니다. 첫 실행에서 그게 안 됐습니다. 결측을 75%까지 넣어도 점수가 99.17에서 97.20으로 2점도 안 떨어졌고, 원인은 지표 셋이 오염 도구가 실제로 만드는 값을 보지 않고 짜여 있던 것이었어요. 같은 날 고친 v2에서는 같은 조건의 점수가 76.34까지 내려갔습니다. 다만 값 오류 오염은 v2에서도 0.04점만 움직여서 문제가 다 풀린 건 아니었어요. 이 글은 그 전후 숫자와 원인 셋, 고친 방법을 그날의 ADR과 실행 로그로 적습니다.
...
## v2로 다시 재니 결측은 21점이 떨어졌습니다

세 데이터셋을 모두 v2로 돌린 실행 로그(`results/02_execution_log.md`, 실행 시각 4월 12일 13시 54분, 59건, 에러 0건, 111초)에서 Telco 값은 이렇게 바뀌었습니다. 엔진이 달라서 오염 없는 기준 점수가 99.17에서 97.65로 움직였으니, 절댓값보다 하락폭을 봐야 합니다.

| 오염 유형 | 엔진 | 0% | 10% | 25% | 50% | 75% | 하락폭 |
|---|---|---|---|---|---|---|---|
| completeness | v1 | 99.17 | 99.16 | 99.03 | 98.65 | 97.20 | 1.97 |
| completeness | v2 | 97.65 | 95.17 | 91.34 | 84.79 | 76.34 | 21.31 |
| consistent_repr | v1 | 99.17 | 92.14 | 92.14 | 92.14 | 92.14 | 고정 |
| consistent_repr | v2 | 97.65 | 96.25 | 94.14 | 90.62 | 87.11 | 10.54 |
| class_balance | v1 | 99.17 | 99.94 | 99.77 | 99.06 | 97.72 | 1.45 |
| class_balance | v2 | 97.65 | 99.48 | 98.75 | 97.50 | 96.25 | 1.40 |
| uniqueness | v1 | 99.17 | 92.51 | 89.17 | 85.84 | 84.17 | 15.00 |
| uniqueness | v2 | 97.65 | 90.99 | 87.65 | 84.32 | 82.65 | 15.00 |
| feature_accuracy | v1 | 99.17 | ERROR | ERROR | ERROR | ERROR | |
| feature_accuracy | v2 | 97.65 | 97.65 | 97.65 | 97.62 | 97.61 | 0.04 |

*표 3. 같은 Telco, 같은 오염에서 v1(ADR-008)과 v2(실행 로그)의 점수.*

결측은 1.97점에서 21.31점으로 내려갔고, 92.14에 고정돼 있던 표현 불일치는 96.25에서 87.11까지 강도를 따라 움직이게 됐습니다. 진행 기록에 적어 둔 예상(결측 75%에서 80점 이하)도 76.34로 맞았어요. 값 오류는 에러 4건이 사라졌지만 점수는 97.65에서 97.61로 0.04점 움직였을 뿐입니다. 클래스 불균형은 계산식을 바꿨는데도 총점 하락폭이 1.45점에서 1.40점으로 그대로였는데, ADR-008이 이 지표를 "가중치가 0.05로 작아 영향 제한적"이라며 뒤로 미룬 이유와 맞는 결과입니다. 그 때문인지 지표 값 자체는 이 글에서 열어 보지 않았어요. 중복은 v1에서도 이미 반응하고 있었고 하락폭은 15점 그대로입니다.

## 이걸로 끝은 아니었습니다

진행 기록에는 "v2 수정 후 completeness 75% 오염이면 80점 이하, 전 등급 분포 생성"이라고 기대를 적어 놨는데, 결측 쪽은 맞았지만 등급 쪽은 아니었습니다. Telco에서 가장 낮은 점수가 76.34로 B였고 C와 D는 나오지 않았어요. 실제로는 v2로 다시 돌린 뒤에도 점수와 모델 성능의 전체 상관계수가 0.08에 머물렀습니다. 결측과 표현 불일치는 이제 보였지만, 값 오류 오염에서 점수가 오히려 올라가는 다른 문제가 남아 있었어요. SouthGermanCredit에서는 값 오류를 넣을수록 97.45가 97.87까지 올랐습니다. 그건 4월 25일 진단에서 잡혔고, 그 이야기는 [다음 글]({{ "/blog/2026/04/25/outlier-metric-self-reference/" | relative_url }})에 적었습니다.
