...
브레이커는 가용성 장치입니다. 프로바이더가 응답을 못 할 때 요청을 빨리 실패시키는 것이지, 돌아온 응답이 맞는지는 전혀 보지 않습니다. 응답 검증은 별도 검증기가 하고, 그 검증기에도 구멍이 있었다는 이야기는 [다른 글]({{ "/2026/04/25/llm-output-validation-korean-tokenizer/" | relative_url }})에 있습니다.
