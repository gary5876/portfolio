...
description: "데이터 품질 점수가 노이즈를 오히려 좋게 읽었다. 빼는 열화만 보던 지표 열 개에 노이즈 지표 하나를 더해 상관을 −0.16에서 +0.95로"
---
...
이미지에서는 점수가 가우시안 노이즈를 "품질 양호"로 읽었습니다. 지표 열 개가 전부 신호를 빼거나 흐리는 열화만 잡게 만들어져 있어서, 노이즈처럼 가짜 신호를 더하는 열화에는 눈이 없었던 거예요. 이 글에는 그게 언제 어떻게 드러났는지, 노이즈를 직접 재는 지표 하나를 어떻게 골라 넣었는지, 그래서 상관이 얼마나 돌아왔는지를 커밋 세 개(bbd88c7, c0c429b, 888d6ff)를 따라 적었습니다.
