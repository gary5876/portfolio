...
안녕하세요, 고준서입니다. 동아리움이라는 대학 동아리 지원 서비스의 백엔드를 팀에서 맡고 있고(백엔드 3인, 프론트엔드 3인), 지원서 도메인과 공지사항 첨부파일은 제가 담당했습니다. 2025년 10월 말에서 11월 초는 이 서비스가 실서비스로 넘어가기 직전이었고, 그 일주일 동안 제가 낸 버그 이슈가 네 개였어요. 하나는 지원자 합불 처리가 특정 조건에서만 실패하는 것, 하나는 대시보드 숫자가 안 맞는 것, 나머지 둘은 첨부파일 링크가 깨지는 것이었는데 이 둘은 원인이 같았습니다.

원인은 삭제 방식, 세는 기준, 설정값으로 서로 달랐지만, 로컬에서 만든 시드 데이터로는 안 보이던 문제라는 점이 같았어요. 이 글은 그 일주일의 네 문제를 PR 본문과 코드 변경으로 적습니다. 다만 넷 다 이슈에 재현 방법만 있고 로그와 스크린샷 칸은 비어 있어서, 처음에 무엇을 의심했는지와 실제 예외 메시지는 기록에 남아 있지 않습니다. 원인 문장은 나중에 PR 본문에 적은 것을 그대로 옮겼고, 고치는 방법을 다른 것과 비교한 기록도 없어요. PR 템플릿의 "고민했던 내용"이 #183과 #185 모두 비어 있습니다. PR에 적은 소요 시간은 #183이 3시간, #185가 1시간이고, 첨부파일 쪽 #193, #201, #205는 각각 10분, 5분, 1분입니다.
...
넷 다 코드보다 데이터가 먼저 드러낸 문제였습니다. 모집 중인 동아리에서만 실패한 건 그 동아리에만 지원서를 가리키는 회원 행이 있었기 때문이고, 대기 지원자 수가 안 맞은 건 회원이 된 사람의 지원서가 아직 테이블에 있었기 때문이고, 링크가 깨진 건 버킷이 둘로 나뉘어 있었기 때문이에요. 로컬에서 만든 시드 데이터로는 안 보이던 것들이 실제 동아리 데이터를 넣고 화면을 눌러 보니 나왔습니다. 그리고 환경변수 이름 하나를 세 번 고친 건, 설정 파일에는 테스트도 리뷰 봇도 없다는 걸 그때 알았기 때문입니다. 지금도 그 파일에는 둘 다 없어요.

이 일주일에서 남은 사실은 이렇습니다.

1. 네 이슈에는 재현 방법만 있고 로그가 없어서, 지금은 그때의 예외 메시지를 다시 볼 수 없습니다.
2. 개별 삭제를 고른 이유와 다른 방법은 기록이 없습니다. 원인은 PR 본문에 있고, 고민은 비어 있습니다.
3. 설정 파일은 테스트와 리뷰 봇 어느 쪽도 보지 않아서, 환경변수 이름 하나를 세 번 고치는 동안 사람 눈이 유일한 검사였습니다.
