...
안녕하세요, 고준서입니다. 카카오테크캠퍼스 세 번째 미션(spring-gift-enhancement)에서 받은 페이지네이션 피드백 이야기예요. 페이지네이션은 목록을 한 번에 다 주지 않고 몇 개씩 나눠 주는 방식인데, 2단계 미션 주제가 이거였습니다. 멘토가 짚어 준 문제 세 가지 중 미션이 끝날 때까지 하나도 고치지 못했고, 그중 하나는 석 달 뒤 팀 프로젝트에서야 적용했습니다. 못 고친 이유는 다음 단계의 필수 요구사항이 먼저였기 때문이고, 그건 PR 본문에 제 손으로 적혀 있어요. 무엇을 듣고 무엇을 못 했는지, 석 달 뒤에는 어떻게 적용했는지를 코멘트와 코드를 인용해 적어 둡니다. 데이터 크기나 쿼리 시간 같은 숫자는 미션에서도 팀 프로젝트에서도 재지 않았습니다.
...
`Slice`는 팀 프로젝트에서도 쓰지 않았습니다. 공지 목록 저장소는 `Page<Notice> findAlive(Pageable pageable)`이고, 화면에서 전체 페이지 수를 보여 주니 `Page`가 맞다고 봤어요. 미션 때 배운 "전체 개수를 보여줘야 하면 Page"라는 기준으로 고른 것이긴 한데, 공지 수가 count 쿼리가 부담될 규모는 아니어서 실제로 재 본 적은 없습니다. 그러니 이 선택이 맞았다는 근거는 화면 요구사항 하나뿐이고, 숫자로는 아직 없습니다.
