...
그중 "친구 추천"을 정하는 데 6월 26일 하루가 걸렸습니다. 결론부터 말하면, 서비스 첫날엔 거의 모든 사용자가 친구 0명이라 공통 친구로는 목록을 채울 수 없고, 그래서 프로필의 목표 유사도를 주력 신호로 두기로 했어요. 이 글에는 오전에 쓴 초안이 어디서 틀렸는지, 무엇을 버리고 무엇을 골랐는지를 그날 커밋 두 개와 결정 기록(ADR)을 따라 적었어요. 코드는 아직 한 줄도 없던 날이라 측정치는 없어요.
