...
description: "처음 서버에 올리던 날 멘토가 던진 질문에 답하고, 넘어갔던 CORS 질문에 지금 답한 기록. 같은 주소라 CORS는 걸리지 않았고, 허용 목록에서 의미 있는 건 하나였다"
---

안녕하세요, 고준서입니다. 카카오테크캠퍼스 마지막 미션(spring-gift-order) 3단계는 지금까지 만든 서비스를 AWS EC2 서버에 직접 올리는 것이었어요. 2025년 8월 1일 오후에 PR([#293](https://github.com/next-step/spring-gift-order/pull/293))을 올렸고, 멘토는 코드를 고치라는 말보다 질문을 더 많이 남겼습니다. 배포 스크립트에 대한 질문에는 답했는데, "우리 앱은 한 서버에서 화면과 API가 같이 도는데 어느 부분에서 CORS가 필요하냐"는 질문에는 "이해했습니다"라고만 달고 미션을 끝냈어요. 지금 답하면 이렇습니다. 같은 주소에서 열린 화면이 같은 주소로 보내는 요청에는 브라우저가 CORS를 적용하지 않으니, 허용 목록에 넣은 주소 셋 중 실제로 의미가 있는 건 개발용 `localhost:8000` 하나뿐이었습니다.

이 글은 그 질문들에 그때 뭐라고 답했고 무엇을 못 답했는지를 리뷰 코멘트 원문으로 적고, 못 답한 것에 지금 답을 붙인 기록입니다. 저장소를 다시 열면서 찾은 것도 하나 있어요. 코멘트를 받고 고친 CORS 경로 패턴이 한 시간 반 동안 아무 요청에도 붙지 않는 상태였습니다. 배포한 서버에 실제 사용자는 없었고 멘토와 저만 접속했으니, 트래픽이나 응답 시간 같은 숫자는 이 글에 없습니다.
...
그날 밤과 다음 날 새벽에 걸쳐 스크립트를 다시 썼습니다. 저녁 5시 반에 파일로 분리하고([f104968](https://github.com/gary5876/spring-gift-order/commit/f104968)), 자정 넘어 빌드까지 넣었어요([c6e9dcd](https://github.com/gary5876/spring-gift-order/commit/c6e9dcd), [18a91ed](https://github.com/gary5876/spring-gift-order/commit/18a91ed)). 빌드를 넣은 건 멘토 말대로 빌드 때 테스트가 같이 돌기 때문이고, 스크립트 하나로 처음부터 끝까지 재현되는 쪽이 제가 손으로 빌드한 파일을 올리는 것보다 실수가 적겠다고 봤어요. 최종 `deploy.sh`는 실행 중인 프로세스를 찾아 종료하고, 저장소를 받아 오고, 빌드하고, 새로 띄우는 네 단계입니다.
...
그런데 "어느 부분에서 필요하냐"는 질문에는 1분 뒤에 "이해했습니다 감사합니다!"라고만 달고 넘어갔어요. 확인해 보라는 말도 확인하지 않았습니다. 멘토는 그날 밤 "답변 달아주신 내용 보니 깊게 잘 공부해보신게 보이네요💯"라고 승인해 주셨지만, 저는 그 질문 하나에는 답을 안 한 채로 미션을 끝냈습니다. 접속이 안 된다는 코멘트가 두 번 있었는데 그건 CORS와 무관했어요. 한 번은 README 주소에 포트를 두 번 적은 오타, 한 번은 EC2 보안 그룹 설정이었고, 밤 11시 39분에 고쳤다는 코멘트가 이 미션의 마지막 코멘트였습니다.
...
그 미션 저장소의 `CorsConfig`는 지금도 그날 새벽 그대로예요. 허용 주소 셋에 `/api/**`. 의미 없는 주소 둘을 빼지 않았고, 멘토가 "확인해 보시겠어요"라고 한 것을 실제로 자기 주소를 빼고 돌려 확인한 기록도 없습니다. 위의 "등록하지 않아도 동작한다"는 브라우저의 동일 출처 규칙에서 나온 답이지, 그 서버에서 직접 확인한 답은 아니에요. 그 서버는 미션이 끝나고 내렸습니다.

이 리뷰에서 지금도 쓰는 기준은 둘입니다. 설정값을 넣을 때 "이 값이 없으면 무엇이 깨지나"를 먼저 말할 수 있어야 한다는 것. 주소 셋 중 둘은 그 답이 없었습니다. 그리고 "확인해 보시겠어요"에 "이해했습니다"로 답하면 그 질문은 닫힌 게 아니라 미뤄진 것이라는 것. 이 글이 그 미뤄진 답입니다.
