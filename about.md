---
layout: page
title: 소개
permalink: /about/
---

**고준서 (Junseo Go)**, 백엔드 개발자

카카오테크캠퍼스 백엔드 과정을 수료했습니다. spring-gift 미션을 단계별 코드리뷰로 진행했고,
그 과정이 Team18_BE 팀 프로젝트로 이어졌습니다.

요즘 관심사는 "AI를 믿는 개발"이 아니라 "AI를 검증하는 개발"입니다. 코딩 에이전트의 결과물이
테스트, 벤치마크, 통계 게이트를 통과했을 때만 받아들이는 워크플로를 만들고 있습니다.

### Stack
Java, Kotlin, Spring Boot, Python, FastAPI, MySQL, PostgreSQL, Redis, Docker, AWS, GCP

### Certificates
- AWS Certified AI Practitioner (2026.08 취득, 2029.08까지 유효)
- AWS Cloud Practitioner Essentials (2026.08 수료)
- 카카오테크캠퍼스 백엔드 과정 (단계별 코드리뷰 미션 + 팀 프로젝트 → 10개월 실서비스 운영)

### Contact
[jerry0622@naver.com](mailto:jerry0622@naver.com), [github.com/gary5876](https://github.com/gary5876)
